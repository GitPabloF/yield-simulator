...
const createSimulationController = async (req, res) => {
    try {
        ...
        if (isNaN(purchasePriceNum) || purchasePriceNum <= 0) {
            return res.status(400).json({ message: "Purchase price must be greater than zero" })
        }

        if (isNaN(monthlyRentNum) || monthlyRentNum < 0) {
            return res.status(400).json({ message: "Monthly rent cannot be negative" })
        }

        if (isNaN(annualRentalFeeNum) || annualRentalFeeNum < 0) {
            return res.status(400).json({ message: "Annual rental fee cannot be negative" })
        }
        ...
        if (managementCommissionYear1 !== undefined && managementCommissionYear1 !== "") {
            const value = Number(managementCommissionYear1)
            if (isNaN(value) || value < 0 || value > 1) {
                return res.status(400).json({ message: "Invalid management commission year 1" })
            }
            params.managementCommissionYear1 = value
        }

        if (managementCommissionYear2 !== undefined && managementCommissionYear2 !== "") {
            const value = Number(managementCommissionYear2)
            if (isNaN(value) || value < 0 || value > 1) {
                return res.status(400).json({ message: "Invalid management commission year 2" })
            }
            params.managementCommissionYear2 = value
        }

        if (managementCommissionYear3Plus !== undefined && managementCommissionYear3Plus !== "") {
            const value = Number(managementCommissionYear3Plus)
            if (isNaN(value) || value < 0 || value > 1) {
                return res.status(400).json({ message: "Invalid management commission year 3 plus" })
            }
            params.managementCommissionYear3Plus = value
        }

        const simulation = await createSimulation(params)
